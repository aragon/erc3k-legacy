...
abstract contract IERC3000 {
    function schedule(uint256 index, address executor, ERC3000Data.Action[] memory actions, bytes memory proof) virtual public returns (bytes32 actionHash);
    event Scheduled(bytes32 indexed actionHash, address indexed actor, address indexed executor, ERC3000Data.Action[] actions, bytes proof, uint256 index, uint256 executionTime, ERC3000Data.Collateral collateral);

    function execute(uint256 index, address executor, ERC3000Data.Action[] memory actions) virtual public returns (bytes[] memory execResults);
